...
public class ListaContactos {
	...
	public void insertarNodoTemporal(PosicionPersona p) {
		...
		while (aux != null) {
			if (aux.getFecha().compareTo(p.getFechaPosicion()) == 0) {
				encontrado = true;
				insertarCoordenadaExistente(aux, p);
				break;
	...
	private void insertarCoordenadaExistente(NodoTemporal nodoTemporal, PosicionPersona p) {
		...
		if (!npEncontrado) {
			...
			insertarNodoPosicionEnLista(nodoTemporal, npAnt, npNuevo);
		}
	}
	...
	private void insertarNodoPosicionEnLista(NodoTemporal nodoTemporal, NodoPosicion npAnt, NodoPosicion npNuevo) {
